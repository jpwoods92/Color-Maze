...
function handleStart () {
  ...
  currentDiv.className = startName + ' ' + 'visited' + ' ' + 'active'
}
...
function toggleActive (coordinates) {
  ...
  if (!currentDiv.className.includes('visited')) {
    currentDiv.className += ' ' + 'visited' + ' ' + 'active'
  } else {
...
function handleMovement (e) {
  ...
  if (e.keyCode === 37) {
    ...
    if (newGrid.grid[posX - 1] !== undefined) {
      coordinates[0] -= 1
      posX -= 1
      if (newGrid.grid[posX][posY].slice(0, 4) === 'blue' || newGrid.grid[posX][posY].slice(0, 3) === 'red') {
        endGame()
